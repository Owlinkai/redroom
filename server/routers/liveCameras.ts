...
export function mapDgtCamera(camera: DgtCamera): LiveCamera | null {
  ...
  return {
    ...
    country: 'Spain',
    feed_url: image,
    external_url: 'https://www.dgt.es/conoce-el-estado-del-trafico/camaras-de-trafico/',
...
export function parseHongKongTrafficCameras(xml: string): LiveCamera[] {
  ...
  for (const raw of xml.split('<image>').slice(1)) {
    ...
    cameras.push({
      ...
      country: 'Hong Kong',
      feed_url: imageUrl,
      external_url: 'https://data.gov.hk/en-data/dataset/hk-td-sm_1-traffic-snapshot-images',
      refresh_interval_ms: PUBLIC_SNAPSHOT_REFRESH_MS,
...
export function mapSingaporeTrafficCamera(camera: SingaporeTrafficCamera): LiveCamera | null {
  ...
  return {
    ...
    country: 'Singapore',
    feed_url: image,
    external_url: 'https://data.gov.sg/datasets?query=traffic%20images',
    refresh_interval_ms: PUBLIC_SNAPSHOT_REFRESH_MS,
